                            ...
                            <tr class="{{ $teacher->deleted_at ? 'disabled-row' : '' }}">
                                ...
                                <td>
                                    <div style="display: flex; gap: 4px;">
                                        <!-- View Button -->
                                        <a href="{{ route('account.teachers.show', $teacher->id) }}" class="btn btn-info"
                                        ...
                                        <!-- Delete Button -->
                                        <button
                                            onclick="deleteTeacher({{ $teacher->id }}, '{{ $teacher->name }}', {{ ($teacher->stages->count() ?? 0) + ($teacher->links->unique('subject_id')->count() ?? 0) + ($teacher->links->unique('class_id')->count() ?? 0) }})"
    ...
    <script>
        ...
        document.getElementById('searchInput').addEventListener('input', function() {
            ...
            rows.forEach(row => {
                ...
                if (teacherName.includes(searchTerm) || username.includes(searchTerm) || email.includes(
                        searchTerm)) {
                    row.style.display = '';
                } else {
