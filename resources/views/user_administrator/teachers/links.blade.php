    ...
    <div class="card">
        ...
        <div class="search-bar">
            ...
            <select id="classFilter" class="form-input">
                ...
                @foreach ($links->unique('class_id') as $link)
                <option value="{{ $link->class->name }}">{{ $link->class->name }}</option>
                @endforeach
            </select>
            <select id="subjectFilter" class="form-input">
                <option value="">All Subjects</option>
                @foreach ($links->unique('subject_id') as $link)
                <option value="{{ $link->subject->name }}">{{ $link->subject->name }}</option>
                @endforeach
            </select>
        </div>
        ...
        <table class="table" id="linksTable">
            ...
            <tbody>
                ...
                <tr
                    data-stage="{{ $link->class->stage->name ?? '' }}"
                    data-class="{{ $link->class->name ?? '' }}"
                    data-subject="{{ $link->subject->name ?? '' }}">
                    ...
                    <td>
                        <div style="display: flex; gap: 4px;">
                            <!-- Delete Button -->
                            <button
                                onclick="deleteclass({{ $link->class_id }}, {{ $link->subject_id }})"
...
<script>
    ...
    document.getElementById('searchInput').addEventListener('input', function() {
        ...
        rows.forEach(row => {
            ...
            if (stageName.includes(searchTerm) || className.includes(searchTerm) || subjectName.includes(searchTerm)) {
                row.style.display = '';
            } else {
